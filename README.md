...
code "6012 research_project"
Replace YOUR_GITHUB_REPOSITORY_URL with the HTTPS link copied from GitHub.
Required VS Code extension
Install the PlatformIO IDE extension from the VS Code Extensions menu. Once the project folder is open, PlatformIO should detect platformio.ini and install the required Arduino Uno platform and PCA9685 library automatically.
The firmware can then be compiled and uploaded using the Build and Upload buttons on the PlatformIO toolbar. Connect the Arduino and use PlatformIO: Serial Monitor at 115200 baud to control the array.
...
cd "6012 research_project"
python scripts/cilia_path_designer.py
...
Command	Action
phase 180	Set the adjacent phase shift to 180 degrees. Valid range: 0–360°. The array must be stopped.
speed 3	Set the complete cycle duration to 3 s. Valid range: 0.5–120 s.
start	Move smoothly into the gait and begin continuous operation. Also resumes from a pause.
pause	Hold the cilia at their current positions.
stop	Stop the gait and return all cilia smoothly to 90 degrees.
home	Same action as stop.
status	Display the current state, cycle duration and phase shift.
